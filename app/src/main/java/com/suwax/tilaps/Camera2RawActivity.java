...
import com.suwax.tilaps.utils.ImageSaver;
import com.suwax.tilaps.view.AutoFitTextureView;
...
public class Camera2RawActivity extends Fragment implements View.OnClickListener {
    ...
    private LinearLayout grid_line;
    static {
    ...
    public int mFlash = 0;
    boolean mManualFocus = false;
    double mFocusDistant = 0.0;
...
//                        }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        initView();
        grid_line = fragment.findViewById(R.id.gridLine);
        return fragment;
    }
    ...
    private void initView() {
        mTextureView = fragment.findViewById(R.id.texture);
...
/*
        switch (view.getId()) {
            ...
            case R.id.btn_iso:
                ...
                setExposureTime(toNanoSec(exp)*0);
                setIso(600);
                setWhiteBalance(1);
                setExposureCompensation(0);
                setFlash(0);
                setManualFocus(true,mFocusDistant-0.1);
 ...
 */
    ...
    public  void setCameraSettingCharacteristic(){
        mPreviewRequestBuilder.set(CaptureRequest.FLASH_MODE, mFlash);//выспышка
//        mPreviewRequestBuilder.set(CaptureRequest.CONTROL_MODE, CaptureRequest.CONTROL_MODE_OFF);
        mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION, mExposureCompensation);//компенсация экспозиции
        mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_OFF);
        mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AWB_MODE, mWhiteBalance );//баланс белого от 0 до 8 - 1 авто - 0 выключен - от 2 до 8 режимы
        mPreviewRequestBuilder.set(CaptureRequest.SENSOR_SENSITIVITY, mISO);//исо светочувствительность
        mPreviewRequestBuilder.set(CaptureRequest.SENSOR_EXPOSURE_TIME, mExposureTime);//выдержка кадра (длительность открытого затвора)
        if (mManualFocus) {
            mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AF_MODE, CaptureRequest.CONTROL_AF_MODE_OFF);
            mPreviewRequestBuilder.set(CaptureRequest.LENS_FOCUS_DISTANCE, Float.valueOf(""+mFocusDistant));
        } else {
            mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AF_MODE, CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE);
            //mPreviewRequestBuilder.set(CaptureRequest.LENS_FOCUS_DISTANCE, 1000f);
        }

        int[] capabilities = mCharacteristics.get(CameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES);
        float yourMaxFocus = mCharacteristics.get(CameraCharacteristics.LENS_INFO_MINIMUM_FOCUS_DISTANCE);
        Log.v("float",""+capabilities.length);

        // mPreviewRequestBuilder.set(CaptureRequest.EXPO, mExposureTime);
    ...
    public void setFlash(int flash) {
        mFlash = flash;
        Log.v("long", flash + " == " + mFlash);
        setCameraSettingCharacteristic();
    }

    public void setManualFocus(boolean manualFocus, double focusDistant) {
        mManualFocus = manualFocus;
        mFocusDistant = focusDistant;
        Log.v("long", mManualFocus + " -> " + mFocusDistant);
        setCameraSettingCharacteristic();
    }

    public void setGridLine(int line){
        switch (line){
            case 0:{

                grid_line.setVisibility(grid_line.INVISIBLE);
                break;
            }
            case 1:{

                grid_line.setVisibility(grid_line.VISIBLE);
                break;
            }
            default:{
                break;
    ...
    public void setWhiteBalance(int whiteBalance) {
        if(whiteBalance < 0){
            mWhiteBalance = 1;
        }else if(whiteBalance > 8){
            mWhiteBalance = 8;
        }else{
            mWhiteBalance = whiteBalance;
        }
        Log.v("long", whiteBalance + " == " + mWhiteBalance);
        setCameraSettingCharacteristic();
    }
    public void setExposureCompensation(int exposure) {
        mExposureCompensation = exposure;
        Log.v("long", exposure + " == " + mExposureCompensation);
        setCameraSettingCharacteristic();
    }
    ...
    public void setExposureTime(long exposureTime){
        mExposureTime = exposureTime;//toNanoSec(exposureTime);
        Log.v("long",mExposureTime+" == "+toNanoSec(exposureTime));
        setCameraSettingCharacteristic();
...
//            setup3AControlsLocked(captureBuilder);
    ...
    private void openCamera() {
        ...
        try {
            ...
            if (!mCameraOpenCloseLock.tryAcquire(60000, TimeUnit.MILLISECONDS)) {
    ...
    private boolean setUpCameraOutputs() {
        ...
        try {
            ...
            for (String cameraId : manager.getCameraIdList()) {
                ...
                synchronized (mCameraStateLock) {
                    // Set up ImageReaders for JPEG and RAW outputs.  Place these in a reference fm
    ...
    private void takePicture() {
       ...
       // setExposureTime(toNanoSec(exp)-1);

        //mExposureTime-=1;
        setCameraSettingCharacteristic();
        Log.v("iso",mExposureTime+"______________"+mISO);
    }
